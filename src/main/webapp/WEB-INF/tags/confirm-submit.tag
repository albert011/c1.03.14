...
<jstl:if test="${test}">
	<jstl:set var="action" value="${acme:pushIdParameter(action, id)}"/>
		<button type="submit" formmethod="${method}" onclick="javascript: if(confirm('¿Estás seguro de que deseas enviar?')){form.action = getAbsoluteUrl('${action}');}" class="btn btn-dark">
